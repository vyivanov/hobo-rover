...
constexpr auto BEACON_PERIOD_MS = BEACON_PERIOD_SEC * 1e3;
constexpr auto HI_PERIOD_MS     = 25.0;
constexpr auto LO_PERIOD_MS     = BEACON_PERIOD_MS - HI_PERIOD_MS;

inline auto pin_initialize() noexcept -> void;
inline auto pin_set_hi(double delay_ms) noexcept -> void;
inline auto pin_set_lo(double delay_ms) noexcept -> void;

inline auto sleep_for(double milliseconds) noexcept -> void;
...
auto pin_set_hi(const double delay_ms) noexcept -> void {
  PORTB |= BEACON_PIN_MASK;
  sleep_for(delay_ms);
}

auto pin_set_lo(const double delay_ms) noexcept -> void {
  PORTB &= (~BEACON_PIN_MASK);
  sleep_for(delay_ms);
}

auto sleep_for(const double milliseconds) noexcept -> void {
